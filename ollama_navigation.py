...
from ollama_client import classify_navigation_intent, select_building_door, select_interactable_target, select_npc_target, select_route_exit
...
def _talk_to_npc_action(ctx: NavContext, world: WorldSnapshot) -> dict | None:
    ...
    # Counter-talk: if dist==2 on same axis with a blocked tile between, treat as adjacent
    counter_talk = False
    if dist == 2 and world.collision:
        grid_w, grid_h, grid_rows = world.collision
        dx = target["x"] - world.player_x
        dy = target["y"] - world.player_y
        if (dx == 0 or dy == 0):  # same row or column
            mid_x = world.player_x + (1 if dx > 0 else -1 if dx < 0 else 0)
            mid_y = world.player_y + (1 if dy > 0 else -1 if dy < 0 else 0)
            if (0 <= mid_x < grid_w and 0 <= mid_y < grid_h
                    and not is_passable(grid_rows[mid_y][mid_x])):
                dist = 1  # treat as adjacent — can talk across counter
                counter_talk = True
    if dist == 1:
        ...
        # For counter-talk, face toward the counter (1 tile away), not the NPC (2 tiles)
        if counter_talk:
            cdx = 1 if target["x"] > world.player_x else -1 if target["x"] < world.player_x else 0
            cdy = 1 if target["y"] > world.player_y else -1 if target["y"] < world.player_y else 0
            face_pos = (world.player_x + cdx, world.player_y + cdy)
        else:
            face_pos = (target["x"], target["y"])
        face_btn = facing_direction((world.player_x, world.player_y), face_pos)
...
def _reachable_route_exits(world: WorldSnapshot, max_candidates: int = 6) -> list[dict]:
    ...
    for segment in segments:
        ...
        if center in seen:
            continue
        seen.add(center)
        path = path_to_target_tile(world.collision, world.player_x, world.player_y, center[0], center[1], max_steps=300)
        if path is None:
            continue
        candidates.append({
            "side": segment["side"],
...
def _select_route_target(ctx: NavContext, world: WorldSnapshot) -> tuple[tuple[int, int] | None, str]:
    ...
    if preferred_side:
        ...
        if preferred:
            ...
            # Preferred exit is unreachable (too far for BFS).
            # Remove exits on the opposite side to avoid bouncing back.
            opposite = {"north": "south", "south": "north", "east": "west", "west": "east"}
            opp = opposite.get(preferred_side)
            if opp:
                non_opposite = [cand for cand in candidates if cand["side"] != opp]
                if non_opposite:
                    candidates = non_opposite
                elif candidates:
                    # Only the bounce-back exit exists — reject it and return
                    # None so the agent free-explores toward the preferred side.
                    return None, f"skipping bounce-back {opp} exit, need to go {preferred_side}"
    if len(candidates) == 1:
        only = candidates[0]
...
def decide_nav_action(ctx: NavContext, world: WorldSnapshot) -> dict | None:
    ...
    if (
        ...
        and ctx.selected_goal_type in ("heal", "shop")
    ):
        intent = "talk_to_npc"
        ctx.intent = intent
    else:
    ...
    if intent == "go_to_route_exit":
        if is_probably_building(world.map_name, world.collision):
            ...
            if target is None:
                # No route exit selected (may have been anti-bounce rejected).
                # Try to walk toward unvisited tiles in the preferred direction
                # instead of bouncing back to the nearest map edge.
                preferred = _preferred_route_side(world)
                if preferred and world.collision:
                    grid_w, grid_h, rows = world.collision
                    # Pick an intermediate waypoint in the preferred direction
                    px, py = world.player_x, world.player_y
                    if preferred == "north":
                        wy = max(py - 30, 0)
                    elif preferred == "south":
                        wy = min(py + 30, grid_h - 1)
                    else:
                        wy = py
                    if preferred == "west":
                        wx = max(px - 30, 0)
                    elif preferred == "east":
                        wx = min(px + 30, grid_w - 1)
                    else:
                        wx = px
                    # Find the nearest passable tile near the waypoint
                    from exploration import path_to_nearest_tile, is_passable
                    path, found = path_to_nearest_tile(
                        world.collision, px, py,
                        lambda x, y, tile, gw, gh, _rows: (
                            is_passable(tile)
                            and abs(x - wx) + abs(y - wy) < 10
                        ),
                        max_steps=200,
                    )
                    step = _follow_path(path)
                    if step:
                        ...
                        ctx.target_pos = found
                        return {
                            "action": step,
                            "reason": f"Structured nav: walking {preferred} toward far exit",
                            "display": f"Heading {preferred} through the route.",
                        }
                # Final fallback: free explore
                ctx.state = NavState.FREE_EXPLORE
                ctx.intent = None
                return None
